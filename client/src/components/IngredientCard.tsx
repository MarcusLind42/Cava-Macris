...
export default function IngredientCard({ name, nutrition, category, color, onAdd }: IngredientCardProps) {
  ...
  return (
    <TooltipProvider>
      <motion.div
        className="ingredient-card glass-panel p-4 rounded-2xl cursor-pointer group"
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-medium text-sm text-white group-hover:text-blue-200 transition-colors">
            {name}
          </h4>
        ...
        <motion.button
          onClick={handleAdd}
          className={`w-full mt-3 py-2 rounded-lg text-xs font-medium transition-all duration-300 ${colorClass} hover:scale-105 active:scale-95`}
          whileTap={{ scale: 0.95 }}
        >
